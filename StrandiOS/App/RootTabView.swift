...
struct RootTabView: View {
    ...
    /// One `NavigationPath` per tab, indexed by tab tag; clearing a path pops that tab to its
    /// root (#135/#198). First-hop links must push `TabRoute`/`MoreDestination` VALUES —
    /// closure-destination links bypass the path.
    ...
    var body: some View {
        ...
        ZStack(alignment: .bottom) {
            ...
            FloatingTabBar(selection: $selectedTab, onReselect: { tag in
                // Re-tap of the active tab: refresh (2026-07-02) and pop to the tab's root
                // (#135); at the root the pop is skipped so scroll position survives (#198).
    ...
    private func pillarScreen(_ dest: NavRouter.Destination) -> some View {
        NavigationStack {
            ...
            // Resolves the Trends/Today fallbacks' TabRoute value pushes in this sheet's stack (#198).
    ...
    private func tab<V: View>(_ view: V, _ title: LocalizedStringKey, _ icon: String,
        ...
        // re-tap of the active tab can pop it to the root (#135/#198).
    ...
    private func moreTab(path: Binding<NavigationPath>) -> some View {
        NavigationStack(path: path) {
            ...
            .toolbar(.hidden, for: .tabBar)   // we draw our own FloatingTabBar
            // Per-screen chrome for every pushed More destination:
...
/// Every screen the More index links to, pushed by VALUE so a re-tap of the More tab can pop it
/// off the path (#135/#198). Per-screen chrome lives at the registration in `moreTab`.
